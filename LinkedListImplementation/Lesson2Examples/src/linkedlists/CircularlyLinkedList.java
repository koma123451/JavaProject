...
public class CircularlyLinkedList<E> implements Cloneable {
 ...
 CircularlyLinkedList<E> other = (CircularlyLinkedList<E>) super.clone(); // safe cast
    if (size > 0) {                    // we need independent chain of nodes
      ...
      while (walk != tail) {              // make a new node storing same element
        newest = new Node<>(walk.getElement(), null);
        otherTail.setNext(newest);     // link previous node to this one
        otherTail = newest;
        walk = walk.getNext();
      }
      newest.setNext(other.tail);
    }
    return other;
  }
 ...
 list.addLast("DDD");
 //
 System.out.println("The initial CircularlyLinkedList is " + list);
 CircularlyLinkedList<String> newList = new CircularlyLinkedList<String>();
 newList= list.clone();
 System.out.println("The cloned CircularlyLinkedList is  " + newList);
